...
def learn():
    epsilon = 0.4 # Epsilon-greedy algorithm in initialized at 1 meaning every step is random at the start
    ...
    model = keras.models.load_model('cartpole.weights')
...
def play():
    ...
    observation = env.reset()
    frames_after_done = 20
    while frames_after_done > 0:
        env.render()
        encoded = observation
        encoded_reshaped = encoded.reshape([1, encoded.shape[0]])
        predicted = model.predict(encoded_reshaped).flatten()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Learn to play the cartpole atari game.')
    parser.add_argument('--learn', help="learn to play the game", action='store_true')
